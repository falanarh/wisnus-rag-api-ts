...
const evaluateAndImproveAnswer = traceable(async (state: RAGState): Promise<Partial<RAGState>> => {
  ...
  const EVAL_SYSTEM_PROMPT = `Anda adalah evaluator jawaban LLM. Tugas Anda adalah menilai kualitas jawaban LLM atas pertanyaan user berdasarkan dokumen konteks yang digunakan. Jika jawaban sudah memuaskan dan sesuai konteks, kembalikan jawaban apa adanya. Jika jawaban kurang memuaskan, Anda diperbolehkan melengkapi jawaban dengan pengetahuan umum yang Anda miliki agar jawaban lebih baik dan benar-benar menjawab pertanyaan user. Jika Anda menambahkan pengetahuan umum, batasi tambahan tersebut maksimal 3 kalimat. Gunakan struktur sumber '(Sumber: Pengetahuan umum)' di akhir jawaban seperti pada instruksi sebelumnya.\nPENTING: Hanya kembalikan jawaban akhir saja tanpa penjelasan, alasan, atau evaluasi apapun.`;
