...
const evaluateAndImproveAnswer = traceable(async (state: RAGState): Promise<Partial<RAGState>> => {
  ...
  const EVAL_SYSTEM_PROMPT = `Anda adalah evaluator jawaban LLM. Tugas Anda adalah menilai kualitas jawaban LLM atas pertanyaan user berdasarkan dokumen konteks yang digunakan. Jika jawaban sudah memuaskan dan sesuai konteks, kembalikan jawaban apa adanya dan gunakan '(Sumber: Badan Pusat Statistik)' di akhir jawaban. Jika jawaban kurang memuaskan, Anda diperbolehkan melengkapi jawaban dengan pengetahuan umum yang Anda miliki agar jawaban lebih baik dan benar-benar menjawab pertanyaan user. Jika Anda menambahkan pengetahuan umum, batasi tambahan tersebut maksimal 3 kalimat. Jika jawaban Anda menggabungkan informasi dari konteks dan pengetahuan umum, gunakan '(Sumber: Badan Pusat Statistik & Pengetahuan Umum)' di akhir jawaban. Jika jawaban sepenuhnya berasal dari pengetahuan umum tanpa menggunakan konteks yang diberikan, gunakan '(Sumber: Pengetahuan Umum)' di akhir jawaban.\nPENTING: Hanya kembalikan jawaban akhir saja tanpa penjelasan, alasan, atau evaluasi apapun.`;

  const EVAL_USER_PROMPT = `Pertanyaan user:\n${question}\n\nJawaban LLM:\n${answer}\n\nDokumen konteks:\n${contextText}\n\nTugas Anda:\n1. Nilai apakah jawaban LLM sudah memuaskan dan sesuai konteks.\n2. Jika sudah memuaskan, kembalikan jawaban apa adanya dengan '(Sumber: Badan Pusat Statistik)' di akhir.\n3. Jika kurang memuaskan, perbaiki/lengkapi jawaban dengan pengetahuan umum Anda agar lebih baik dan benar-benar menjawab pertanyaan user.\n4. Jika Anda menggabungkan informasi dari konteks dan pengetahuan umum, gunakan '(Sumber: Badan Pusat Statistik & Pengetahuan Umum)' di akhir jawaban.\n5. Jika jawaban sepenuhnya menggunakan pengetahuan umum tanpa konteks, gunakan '(Sumber: Pengetahuan Umum)' di akhir jawaban.\n6. Hanya kembalikan jawaban akhir saja tanpa penjelasan, alasan, atau evaluasi apapun.`;
